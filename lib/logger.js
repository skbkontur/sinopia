    ...
    if (target.type === 'stdout' || target.type === 'stderr') {
      ...
      if (target.format === 'pretty') {
        ...
        stream.write = function(obj) {
          ...
          dest.write(print(timestamp, obj.level, obj.msg, obj, dest.isTTY) + '\n')
        }
      } else {
        stream.write = function(obj) {
...
function print(timestamp, type, msg, obj, colors) {
  ...
  if (colors) {
    return timestamp + ' \033[' + levels[type] + 'm' + (pad(type)) + '\033[39m ' + sub + ' ' + finalmsg
  } else {
    return ' ' + timestamp + (pad(type)) + ' ' + sub + ' ' + finalmsg
  }
}
